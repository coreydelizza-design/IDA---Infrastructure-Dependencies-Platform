...
- **Site Inventory interactions** (see `docs/SITE_INVENTORY_INTERACTIONS.md`): a hover/focus **risk peek** (transient portal popover on the open-risks chip) and opt-in inspector layouts (Administration → Site inventory layout; default `docked`) — **overlay** (grid stays full width) and **fullscreen** (selecting a site projects a large, readable card via a portal to `document.body`, OUTSIDE the scaled `.app-shell`, so text stays legible on small displays; `src/components/FullscreenSiteCard.tsx`). All additive — the default docked, no-hover render is pixel-identical to the locked baseline. Do not add new controls to the locked hero; keep such toggles in Administration.
...
- **Site workloads** (see `docs/WORKLOADS.md`): the categories of business/network traffic a location carries (AI, payment, store/POS, voice, SCADA, backup, …). A documented dependency fact — NOT live traffic measurement (no NetFlow/utilisation; stays within the assurance, not monitoring, model). Catalog + archetype presets in `src/domain/workloads.ts`; `workloads: string[]` (WorkloadId[]) on the site aggregate. Captured as grouped checkbox chips in the intake wizard's Business Context step, pre-ticked from archetype presets. Displayed as a conditional Workloads section in the inspector + fullscreen card (rendered only when non-empty — canonical sites are seeded empty, so the locked DC1 – London inspector is unchanged and no re-baseline is needed). Schema v7 backfills the field. Capturing a workload does not change the score.
- **Access enforcement** (see `docs/ACCESS_AND_NAVIGATION.md`): customer (read-only) hides operator affordances via `capabilities.canOperate` — Add Site, the detail-pane action menu (`DetailPane canOperate` prop), and "New contract". The consultant default is unchanged. Not-yet-built routes render a "planned workspace" placeholder with a home link; removing locked sidebar/top-nav items requires a product-owner lock amendment (not done implicitly).
- **Governed customer actions** (see `docs/GOVERNED_ACTIONS.md`): the customer's narrow write path. A governed action records a `CustomerDecision` (approval/input) for consultant reconciliation — it NEVER edits canonical LOAs, risks, sites, or scores. Role-gated by `CustomerRole` (Sponsor/Approver → `canApprove`; Contributor → `canContribute`; Reviewer → neither). Surfaces only as the customer-only "Actions required" panel on the Dashboard (`src/domain/governance.ts`, schema v5 `customerDecisions`). Never a legal-compliance instrument or a live reading; never an operator tool. The consultant render is unchanged.
- **Consultant reconciliation** (see `docs/GOVERNED_ACTIONS.md`, operator half): the ONLY path from a `CustomerDecision` to canonical state. `resolveDecisionEffect` is the single source of truth (LOA approved → authorization active; risk accepted → risk status accepted; any decline → no canonical change). `reconcileDecision` applies the effect, marks the decision `reconciled`, and writes a `decision-reconciled` audit event. Surfaces only as the consultant-only "Reconciliation queue" panel on the Dashboard (`capabilities.canOperate`). Accepting a risk does NOT improve the technical score or erase the gap. Treat as security-sensitive (canonical + audit change).
